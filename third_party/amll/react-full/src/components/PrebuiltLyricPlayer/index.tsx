...
const PrebuiltMediaButtons: FC<{
	...
	return (
		<>
			{showOtherButtons && (
				<MediaButton aria-label="随机播放" aria-pressed={isShuffleOn} className={styles.songMediaButton} onClick={toggleShuffle}>
					{isShuffleOn ? (
			...
			<MediaButton
				aria-label={musicIsPlaying ? "暂停" : "播放"}
			...
			{showOtherButtons && (
				<MediaButton aria-label="单曲循环" aria-pressed={currentRepeatMode === RepeatMode.One} className={styles.songMediaButton} onClick={cycleRepeat}>
					{renderRepeatIcon()}
				</MediaButton>
			)}
		</>
 ...
 * 已经部署好所有组件的歌词播放器组件，在正确设置所有的 Jotai 状态后可以开箱即用
 */
export const PrebuiltLyricPlayer: FC<HTMLProps<HTMLDivElement> & { backgroundCover?: string }> = ({
	className,
	backgroundCover,
	...rest
}) => {
			...
			<AutoLyricLayout
				...
				backgroundSlot={
					...
					backgroundRenderer.renderer === "css-bg" ? (
						...
						<BackgroundRender
							album={backgroundCover ?? musicCover}
