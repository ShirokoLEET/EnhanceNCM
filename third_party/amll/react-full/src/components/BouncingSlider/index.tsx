...
export const BouncingSlider: React.FC<SliderProps> = ({
	...
	useEffect(() => {
		...
		const newProgress = max > min ? Math.max(0, Math.min(1, (value - min) / (max - min))) : 0;
		progressMv.set(newProgress);
	}, [value, min, max, progressMv]);

	useAnimationFrame((_time, delta) => {
		if (isPlaying && max > min && !isDraggingRef.current) {
	...
	return (
		<motion.div
			...
			onKeyDown={(event) => {
				...
				if (next !== null) { event.preventDefault(); onChange?.(Math.max(min, Math.min(max, next))); }
			}}
			ref={containerRef}
